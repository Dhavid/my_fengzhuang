    ...
    *    boundary：是否进行边界校验 （默认值：true，不能超过要移动元素所在容器的范围，需要开发者保证：
    当前移动的元素是相对于它所在容器定位的）   
    * 
    *    声明周期函数（钩子函数）
    *    dragstart：拖拽开始
    ...
    ~ function () {
        ...
        */
        class Drag {
            ...
            // 参数初始化（尽可能把一切信息都挂载到实例上，这样在其它方法中，只要能获取到实例，这些信息都可以
        // 调用 => 我们尽可能保证每个方法中的 THIS 都是实例）
            ...
            move(ev) {
                let {_element, _boundary, startX, startY, startL, startT} = this;
