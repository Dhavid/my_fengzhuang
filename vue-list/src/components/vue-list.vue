...
export default {
  ...
  methods: {
    initData() {
      // init all data
      this._rowsInWindow = Math.ceil(this.$el.offsetHeight / this.height), //一屏显示的条目数
      this._above = this._rowsInWindow * 2, //滚动区域最大可显示的条目数
      this._below = this._rowsInWindow,  //页面下方可显示的条目数
      this._max = this._rowsInWindow * this.height; //一屏的高度
    },
    handleScroll() {
