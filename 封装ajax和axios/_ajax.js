    ...
    */
    ~ function () {
        ...
        let defaults = {
            ...
            data: null, // 传递给服务器的信息：支持格式 STRING 和 OBJECT，如果是 OBJECT，我们需要把其处理
        // 为 x-www-form-urlencoded 格式；GET 请求是把信息作为问号参数传递给服务器，POST 请求是放到请
        // 求主体中传递给服务器；
            dataType: 'JSON', // 把服务器返回结果处理成为对应的格式 JSON / TEXT / XML
            async: true, // 是否异步请求
            cache: true, // 只对 GET 请求有作用：设置为 FALSE,在 URL 的末尾加随机数来清除缓存
            timeout: null, // 超时时间
            headers: null, // 设置请求头信息（请求头信息不能是中文，所以我们需要为其编码）
            success: null, // 从服务器获取成功后执行 把获取的结果、状态信息、XHR 传递给它
