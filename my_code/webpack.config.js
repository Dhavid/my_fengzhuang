...
module.exports={
    // __dirname: 当前simple文件夹的绝对路径（当前js文件的绝对路径）
    ...
    module:{
        rules:[
            ...
            // 如果图片的大小超过了url的limit限制，默认就会调用file-loader去处理该图片，此时会生成一个新的图片而不是转base64的编码了；
            {
                ...
                test:/\.less$/,
                // 1.这个顺序不能颠倒 ，webpack会从后往前依次进行解析；
                // 先用less-loader把less文件解析成css,将css注入到js文件中，style-loader创建style标签，把js中的css放进去；
