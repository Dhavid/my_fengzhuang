...
app.use(session({
  resave: true, //true是强制保存，如果session没有被修改也要重新保存
  saveUninitialized: true, //如果原先没有session那么就设置，否则不设置
  secret: 'zhufeng' //设置签名秘钥  内容可以任意填写
}));
...
// app.use(session({
//     secret: 'ZFPX2', //设置签名秘钥  内容可以任意填写
//     saveUninitialized: true, //如果原先没有session那么就设置，否则不设置
//     resave: false, //true是强制保存，如果session没有被修改也要重新保存
//     cookie: {
//         // expires:new Date(Date.now() + 20 * 60 * 1000),
//         maxAge: 1000 * 60 * 60 * 24 * 30 //设置cookie的过期时间，例：30天后session和相应的cookie失效过期
//     }
// }));
...
app.use((req, res, next) => {
  ...
  res.header("Access-Control-Allow-Credentials", true); //设为true表示服务器允许发送cookie
  res.header("Access-Control-Allow-Headers", "Content-Type,Content-Length, Authorization, Accept,X-Requested-With");
  res.header("Access-Control-Allow-Methods", "PUT,POST,GET,DELETE,OPTIONS");
  // post请求中，在请求发送之前，一般都会先发送一个options试探请求，目的是验证当前请求能否和服务器建立连接，能连接上再发送正式请求
...
app.post('/login', (req, res) => {
  ...
  if (user) {
    ...
    req.session.user = user.userid;
    userID = user.userid;
    console.log("登录接口",req.session.user);
    res.json({user: user.userid, msg: '', success: 'ok', err: 0});
    tempCode = '!@#$%^&*()_+';
  } else {
...
app.delete('/signout', (req, res) => {
  console.log("退出登录",req.session.user)
...
app.post('/userInfo', (req, res) => {
  console.log("获取个人信息",req.session.user);
  if (!req.session.user) {
...
app.get('/validate', (req, res) => {
  ...
  if (req.session.user) {
    ...
    res.json({user: null, msg: "未登录3333333", success: '', err: 1})
  }
});
